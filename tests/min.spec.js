...
describe ("min", () => {
  it ('should return result of min', () => {
    const mmheap = new MinMaxHeap();

    const input = [4, 6, 1, 2, 7, 3, 5, 9, 8, 10];
    const expected = [4, 4, 1, 1, 1, 1, 1, 1, 1, 1];
    let output = [];
    for (let i = 0; i < input.length; i++) {
        mmheap.push(input[i]);
        output.push(mmheap.min());
    }
    assert.equal(output.toString(), expected.toString());
  });

  it ('should return result of min (100 elements)', () => {
    const mmheap = new MinMaxHeap();
